...
bin_dir="${new_files_dir}bin/"
lib_dir="${new_files_dir}lib/"
mkdir -p "${new_files_dir}" "${inf_dir}" "${lpd_dir}" "${printer_dir}" "${ppd_dir}" "${bin_dir}" "${lib_dir}"
...
cp -av * "${ppd_dir}"
cd "${WORKSPACE}/extraction/usr/bin/"
cp -av * "${bin_dir}"
cd "${WORKSPACE}/extraction/usr/lib/"
cp -av * "${lib_dir}"
cd "${old_pwd}"
