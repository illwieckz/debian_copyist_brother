...
PageScanStart( Brother_Scanner *this )
{
	...
	if( this->scanState.nPageCnt == 1 ){
		...
		if( !bTxScanCmd ){
			...
			//
			nCmdStrLen = MakeupScanStartCmd( this, (LPSTR)szCmdStr );
			if (WriteDeviceCommand( this->hScanner, 
						(LPBYTE)szCmdStr, //M-LNX-105 
						nCmdStrLen, 
		...
		//
		if (WriteDeviceCommand( this->hScanner, 
				      (LPBYTE)MFCMD_SCANNEXTPAGE, //M-LNX-105 
				      strlen( MFCMD_SCANNEXTPAGE ), 
				      this->modelInf.seriesNo))
			rc = TRUE;
	...
	*lpFwLen = 0;
	...
	if ( !this->scanState.bReadbufEnd ) {
		for (rc=0 ; wData < nMinReadSize;)
		{
			...
			if (rc == 0) {                              //M-LNX-159
			...
			else if (rc < 0) {                          //M-LNX-159
				this->scanState.bReadbufEnd = TRUE; //M-LNX-159
				WriteLog( "  bReadbufEnd =TRUE" );  //M-LNX-159
				...
				sc = StatusChk((char *)lpRxBuff, wData);
			  ...
			  break;
				...
				*(pData + length*2) = (*(pData + length*2) & 0xE3) | 0x0C;
				...
				for(i=0;i < length-3; i++){
				  ...
				  if(G > 255) G=255;
				  if(G < 0) G=0;
				  if(B > 255) B=255;   //M-LNX-165 FR02520 G=>B
				  if(B < 0) B=0;       //M-LNX-165 FR02520 G=>B
	...
	nAnswer = ProcessMain( this, wData, wDataLineCnt, 
			       (char *)lpFwTempBuff+FwTempBuffLength, 
			       &FwTempBuffLength, &wProcessSize );
...
PageScan( Brother_Scanner *this, char *lpFwBuf, int nMaxLen, int *lpFwLen )
{
	...
	*lpFwLen = 0;
	...
	// 
	memmove( lpRxBuff, lpRxTempBuff, dwRxTempBuffLength );	//
	...
	if ((dwRxTempBuffLength > 0) || (wProcessSize < wData)) {
		dwRxTempBuffLength += (wData - wProcessSize); 
		memmove( lpRxTempBuff, lpRxBuff+wProcessSize, dwRxTempBuffLength );	//
	}
	...
	memmove( lpFwBuf, lpFwTempBuff, *lpFwLen);	//
	memmove( lpFwTempBuff, lpFwTempBuff+*lpFwLen, FwTempBuffLength ); //
...
//
//-----------------------------------------------------------------------------
//	AbortPageScan¡ part of PageScan i the windows version
void
...
#else  // M-LNX-115
		//
		// When the scan-source is ADF restrict to 17 inch.
		//
		lMaxScanRaster = 17 * this->devScanInfo.DeviceScan.wResoY;
#endif  // M-LNX-115
	}
	//
		  ...
		  answer = SCAN_DUPLEX_REVERSE;
			...
			if( Header == 0 ){
				...
				if( lpFwBuf ){
					...
					ImgLineProcInfo.pWriteBuff     = (LPBYTE)lpFwBuf;   //M-LNX-105
					//
