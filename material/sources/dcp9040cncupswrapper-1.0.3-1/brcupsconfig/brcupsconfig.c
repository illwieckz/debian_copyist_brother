...
{
    char *printer;
    char ppd_line[500],tmp[500],tmp_n[10],tmp_op[500];
    FILE *fp_ppd;
    char *p;
...
//	sleep(100);
    	...
    	for ( i = 0; ppdcommand_all_list[i]!= NULL; i ++){
			...
			if(p){
				...
				for (ii = 0; ppdcommand_all_list[i]->ppdcommandlist[ii].value != NULL; ii++){
					...
					p = strstr_ex(ppd_line+strlen(ppdcommand_all_list[i]->label),ppdcommand_all_list[i]->ppdcommandlist[ii].value);
					if(p){
	...
	//************************************
	//  set CUPS command line option (string)
	//************************************
	for ( i = 0; standard_cups_commandlinelist[i].value != NULL; i ++ )
	{
		p = strstr_ex(commandline, standard_cups_commandlinelist[i].option);
		if(p)
		{
			add_command_list_brcommand(standard_cups_commandlinelist[i].value);
		}
	}
	...
	write_log_file(5,"BROTHER COMMAND LINE SETTING(S) : %s\n" , commandline);
...
int exec_brprintconf(char *brprintconf,char *printer){
  ...
  for ( i = 0; command_array[i].value[0] != 0; i ++ ){
    sprintf(exec,"%s%s  %s  %s",brprintconf,printer,
